...
$recherche = $db->query("SELECT * FROM produits WHERE type_produit LIKE 'Homme' ORDER BY id DESC " );
$recherche = $db->query("SELECT * FROM produits INNER JOIN boutique ON produits.id_boutique = boutique.id WHERE produits.type_produit LIKE 'Homme' ORDER BY produits.id DESC " );
$homme = 'Homme';
...
if(isset($_GET["recherche"]) AND !empty($_GET["recherche"])){
    ...
    $recherche = $db->query("SELECT * FROM boutique INNER JOIN  produits ON produits.id_boutique = boutique.id WHERE produits.type_produit LIKE 'Homme' and boutique.nom_boutique LIKE  '%$rech%'   ORDER BY produits.id DESC " );
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <section class="info_article">
        <img src="Homme_publ.png" alt="">
        <h1>KEPHALÉ VOUS OFFRE LES MEILLIEUR <br> BOUTIQUES POUR HOMMES </h1>
    </section>

<?php
$passwor_usre = sha1($_POST["password_user_un"]);

if(!empty($_POST ["password_user_un"])){
    ...
    if($passwor_usre  == $exist_user["mot_de_passe"]){
        if($exist_user["solde"] >= $montant_paye ){
            ...
            $livre = 'nom';
            $inser_achat = $db->prepare("INSERT INTO liste_achat ( id_user, id_article, id_boutique, quantite_article, taille_article, montant_paye, livre, temp_livraison ) VALUES (?,?,?,?,?,?,?,?)");
            $inser_achat -> execute(array($_SESSION["id"], $id_article, $id_boutique, $quantite_article, $taille_article, $montant_paye, $livre, $temp_livraison ));
            ...
            $erreure = 'Votre solde est insufisant!';
        }
    }else{
    $erreure = 'Mot de passe incorecte!';
    }
}else{

    $erreure = 'Entre votre mot de passe !';
}
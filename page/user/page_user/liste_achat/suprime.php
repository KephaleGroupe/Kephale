...
$inser_historique = $db->prepare("INSERT INTO historique_achat ( id_user, id_article, id_boutique, type_boutique, quantite_article, taille_article, montant_paye, boutique_suprime, user_suprime ) VALUES (?,?,?,?,?,?,?,?,?)");
$inser_historique -> execute(array($result_liste["id_user"], $result_liste["id_article"], $result_liste["id_boutique"], $result_select_boutique["type_abonnement"], $result_liste["quantite_article"], $result_liste["taille_article"], $result_liste["montant_paye"], $boutique_suprime, $user_suprime ));
$oui = 'oui';
$modif_etat = $db->prepare("UPDATE liste_achat SET client_etat = ? WHERE id = ? ");
